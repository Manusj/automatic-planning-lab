...
def main():
    ...
    parser.add_option("-s", "--carrier-space", metavar = 'NUM', dest = "carrier_space", type = int,
                      help='max number of crates in carrier')
    ...
    if options.carrier_space <= 0 and options.carriers > 0:
        print("Carrier free space cannot be zero or less than zero")
        sys.exit(1)
    ...
    print("Carrier space\t\t", options.carrier_space)
    ...
    carrier: list[str] = []
    location: list[str] = []
    num: list[str] = []         # store sequence of numerical object
    ...
    with open(problem_name + ".pddl", 'w') as f:
        ...
        for objname in location:
            f.write("\t(location " + objname + ")\n")

        for objname in content_types:
            f.write("\t(content " + objname + ")\n")

        for objname in carrier:
            f.write("\t(carrier " + objname + ")\n")

        for i in range(len(crates_with_contents)):
            f.write("\t(crates " + content_types[i] + " " + str(len(crates_with_contents[i])) + " depot)\n")

        # TODO: Initialize all facts here!
        for objname in uav:
            f.write("\t(at " + objname + " depot)\n")
            f.write("\t(heli-free " + objname + ")\n")


        for objname in carrier:
            f.write("\t(at " + objname + " depot)\n")
            f.write("\t(space " + objname + " "+ str(len(num)) + ")\n")
            for i in range(len(content_types)):
                f.write("\t(contains " + objname + " "+ content_types[i] + " 0" +")\n")

        need = {}
        for loc in location:
            need[loc] = 0

        for i in range(options.goals):
            randloc = location[random.randint(1, len(location)-1)]
            need[randloc] = need[randloc] + 1

        num_crates = []
        for i in range(len(crates_with_contents)):
            num_crates.append(len(crates_with_contents[i]))

        for loc, num in need.items():
            while num!=0:
                randomtype = random.randint(0, len(content_types)-1)
                #print(randomtype)
                if(num_crates[randomtype] >= num):
                    f.write("\t(need " +" " + content_types[randomtype] + " " + str(num) + " " + loc +")\n")
                    num_crates[randomtype] = num_crates[randomtype] - num
                    num = 0
                elif(num_crates[randomtype] != 0):
                    f.write("\t(need " +" " + content_types[randomtype] + " " + str(num_crates[randomtype]) + " " + loc +")\n")
                    num = num - num_crates[randomtype]
                    num_crates[randomtype] = 0
